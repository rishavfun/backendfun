...
router.get("/sol1", function (req, res) {
    ...
    let missingNumber
    // last no is 7 so n= 7;
    // sum of n natural no starting from 1 =n*(n+1)/2
    let n = 7
    let sum = (n * (n + 1)) / 2 //28
    // console.log(sum);
    let arrSum = 0
    for (i = 0; i < arr.length; i++) {
        arrSum += arr[i] // arrSum = arrSum +  arr[i]
    }
    // console.log(arrSum);
...
router.get("/sol2", function (req, res) {
    ...
    let missingNumber
    // one no in array is missing so total count of element in arr must be arr.length +1
    let n = arr.length+1
    
    let sum1 = (n*(arr[0]+arr[n-2]))/2
    let arrSum = 0
    for (i = 0; i < arr.length; i++) {
        arrSum += arr[i]
    }
    missingNumber = sum1 - arrSum
        console.log(missingNumber);    
