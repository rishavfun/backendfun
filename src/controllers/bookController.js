...
const getBooksInYear = async function (req, res) {
    let allBooks = await BookModel.find({ year: 2022 }).select({ bookName: 1, authorName: 1, _id: 0 })
    res.send({ msg: allBooks })
}

const ParticularBooks = async function (req, res) {
    let allBooks = await BookModel.find( {stockAvailable:true , totalPages:262 })
    res.send({ msg: allBooks })
}

const XINRBooks = async function (req, res) {
    let allBooks = await BookModel.find( {   "prices.indianPrice": { $in: ["100INR" ,"200INR", "500INR"] }  } ).select({ bookName: 1, authorName: 1, _id: 0,prices: 1 })
    res.send({ msg: allBooks })
}
